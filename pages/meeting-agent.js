...
const STEPS = { CALIBRATE: 'calibrate', TARGET: 'target', LOADING: 'loading', REPORT: 'report' };
...
export default function MeetingAgent() {
  ...
  const runResearch = async () => {
    ...
    const hasUrls = linkedinUrl || companyUrl;
    const statuses = hasUrls ? [
      '// FETCHING LINKEDIN PROFILE...',
      '// SCRAPING COMPANY WEBSITE...',
      '// CROSS-REFERENCING WEB DATA...',
      '// ANALYSING INTELLIGENCE...',
      '// SYNTHESISING BATTLECARD...',
    ] : [
      '// SCANNING PUBLIC WEB DATA...',
    ...
    <Layout>
      <div className="page">
        ...
        <div className="header">
          ...
          <h1>Intelligence <span className="highlight">Briefing.</span></h1>
          <p className="subtext">
            ...
            {step === STEPS.REPORT && `Battlecard ready for ${targetName}.`}
          </p>
        </div>
          ...
          <div className="card">
            ...
            <div className="form-grid">
              <div className="field">
                ...
                <input type="text" placeholder="e.g. Founder, Head of Sales" value={calibration.role}
                  onChange={e => setCalibration(p => ({ ...p, role: e.target.value }))} />
              </div>
              <div className="field">
                <label>Your Company</label>
                <input type="text" placeholder="e.g. LearnLab Media" value={calibration.company}
              ...
              <div className="field full">
                <label>Your Context <span className="optional">(optional — the more detail, the sharper the report)</span></label>
                <textarea placeholder="e.g. I specialise in content creation and AI tools for B2B companies. I pitch media packages and custom tool builds." value={calibration.context}
                  onChange={e => setCalibration(p => ({ ...p, context: e.target.value }))} rows={3} />
              </div>
            </div>
        ...
        {step === STEPS.TARGET && (
          <div className="card">
            ...
            <p className="card-desc">
              Add LinkedIn and company URLs for a high-confidence report. Without them the agent falls back to web search only.
            </p>
            ...
            <div className="form-grid">
              ...
              <div className="field">
                <label>
                  ...
                  <span className="url-badge">HIGH IMPACT</span>
                </label>
            ...
            {!linkedinUrl && !companyUrl && (
              <div className="warning-bar">
                // NO URLS PROVIDED — report will use web search only. Confidence will be lower.
              </div>
            )}
        ...
        {step === STEPS.LOADING && (
          <div className="card loading-card">
            ...
            <div className="loading-note">
              {linkedinUrl || companyUrl ? 'Fetching direct sources — this produces the best results.' : 'Running web searches — add URLs next time for higher confidence.'}
            </div>
          </div>
        )}
          ...
          <div className="report">
            <div className="report-header">
              <div>
                ...
                {battlecard.data_quality && (
                  <div className="data-quality">{battlecard.data_quality}</div>
                )}
              </div>
                ...
                <div className="approach-grid">
                  ...
                  {battlecard.suggested_approach.avoid?.length > 0 && (
                    <div className="approach-block">
                      ...
                      {battlecard.suggested_approach.avoid.map((a, i) => (
                ...
                <div className="items-list">
                  {battlecard.questions_to_ask?.map((q, i) => (
                    <div key={i} className="item-card question">
                      ...
                      <div className="item-content">
                        <div className="item-title">{q}</div>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {battlecard.sources_used?.length > 0 && (
              <div className="sources-bar">
                // SOURCES: {battlecard.sources_used.join(' · ')}
              </div>
            )}

            <div className="report-actions">
              <button className="btn-action" onClick={copyReport}>
        ...
        <style jsx>{`
          ...
          .eyebrow { font-family: 'JetBrains Mono'; color: var(--accent); font-size: 11px; letter-spacing: 0.2em; margin-bottom: 16px; }
          ...
          label { font-family: 'JetBrains Mono'; font-size: 10px; letter-spacing: 0.15em; color: var(--text-2); text-transform: uppercase; display: flex; align-items: center; gap: 8px; }
          ...
          .warning-bar {
            ...
            color: var(--warm); background: rgba(255, 107, 53, 0.08);
          ...
          .report-meta { font-family: 'JetBrains Mono'; font-size: 11px; color: var(--text-2); letter-spacing: 0.1em; margin-bottom: 8px; }
          ...
          .btn-action:hover { background: rgba(255,255,255,0.08); color: #fff; border-color: rgba(255,255,255,0.2); }
...
function generatePrintHTML(b, name, company) {
  ...
  <div class="meta">${b.person?.role || ''} · ${b.company?.name || company} · ${b.data_quality || ''}</div>
  <h2>Person Overview</h2><p>${b.person?.background || ''}</p>${(b.person?.personality_signals || []).map(s => `<span class="tag">${s}</span>`).join('')}
  ...
  <h2>Suggested Approach</h2><div class="item"><div class="item-title">Opening</div><div>${b.suggested_approach?.opening || ''}</div></div><div class="item"><div class="item-title">Angle</div><div>${b.suggested_approach?.angle || ''}</div></div><div class="item"><div class="item-title">Tone</div><div>${b.suggested_approach?.tone || ''}</div></div>
  <h2>Questions to Ask</h2><ul>${(b.questions_to_ask || []).map(q => `<li>${q}</li>`).join('')}</ul>
  <h2>Risk Flags</h2>${(b.risk_flags || []).map(r => `<div class="item"><div class="item-title">⚠ ${r.flag}</div><div class="item-sub">Mitigation: ${r.mitigation}</div></div>`).join('')}
  </body></html>`;
}