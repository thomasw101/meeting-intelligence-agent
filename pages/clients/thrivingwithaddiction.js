...
export default function ThrivingWithAddiction() {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 130; i++) {
      particles.push({
        ...
        size: Math.random() * 1.5 + 0.3, warm: Math.random() > 0.6,
    ...
    const animate = () => {
      ...
      particles.forEach(p => {
        ...
        ctx.fillStyle = p.warm ? `rgba(160,100,60,${p.opacity})` : `rgba(74,124,126,${p.opacity})`;
        ctx.globalAlpha = 1;
        ctx.fill();
      });
  ...
  const handleSubmit = async () => {
    ...
    setError(''); setClips([]); setExpandedClip(null); setLoading(true);
    try {
  ...
  return (
    <div className="wrap">
          ...
          <div className="inner">
            ...
            <div className="pkg-grid">
              <div className="pkg-main fade-up">
                ...
                <div className="pkg-items">
                  {deliverables.map((item, i) => (
                    <div key={i} className={`pkg-item ${expandedDeliverable === i ? 'pkg-item-expanded' : ''}`} onClick={() => setExpandedDeliverable(expandedDeliverable === i ? null : i)}>
                      ...
                      {expandedDeliverable === i && <div className="pkg-item-detail"><p>{item.detail}</p></div>}
                    </div>
                  ))}
                </div>
              ...
              <div className="pkg-side">
                {[
                  { key: 'trailer', label: 'The Guest Trailer Concept', cls: '', summary: "Before each episode drops, we put out a 60 to 90 second cut built around your guest's world. UFC highlights for Jared Gordon. ESPN clips for Lauren Sisler.", detail: "It introduces the guest to your audience and introduces your podcast to theirs. The episode hasn't even dropped yet and you already have content out that their fans will share. It also gives the guest something to repost — which doubles your reach before a single listener has tuned in." },
                  { key: 'book', label: 'What This Does For the Book', cls: 'side-card-teal', summary: 'Every clip drives people to the full episode. Every full episode drives people to Thriving with Addiction.', detail: 'Someone finds a 45 second clip on Instagram, watches it, follows the account, listens to three full episodes, and buys the book. That chain starts with a single well-cut clip. The content becomes a discovery engine — running continuously, compounding over time.' },
                  { key: 'flywheel', label: 'The Flywheel Effect', cls: 'side-card-flywheel', labelCls: 'side-card-label-orange', summary: 'More content means more eyeballs. More eyeballs means a bigger platform. A bigger platform attracts bigger guests — and bigger guests bring their audiences with them.', detail: "This is how shows go from a few thousand listeners to tens of thousands without running ads. The content does the work — it circulates, gets shared, surfaces in search, lands in front of people who have never heard of you. Each episode compounds on the last. The earlier the flywheel starts spinning, the harder it becomes to stop." },
                ].map(card => (
                  <div key={card.key} className={`side-card fade-up ${card.cls} ${expandedDeliverable === card.key ? 'side-card-open' : ''}`} onClick={() => setExpandedDeliverable(expandedDeliverable === card.key ? null : card.key)}>
                    <div className="side-card-header">
                      <div className={`side-card-label ${card.labelCls || ''}`}>{card.label}</div>
                      <div className="pkg-chevron">{expandedDeliverable === card.key ? '−' : '+'}</div>
                    </div>
                    <p>{card.summary}</p>
                    {expandedDeliverable === card.key && <p className="side-card-detail">{card.detail}</p>}
                  </div>
                ))}
              </div>
            </div>
        ...
        <section id="sec-why" className="section">
          <div className="inner">
            ...
            <div className="reasons-grid">
              {reasons.map((r, i) => (
                <div key={i} className={`reason-card fade-up ${expandedReason === i ? 'reason-expanded' : ''}`} onClick={() => setExpandedReason(expandedReason === i ? null : i)}>
      ...
      <style jsx>{`
        ...
        .tool-card { background: #fff; border: 1px solid rgba(74,124,126,0.15); border-radius: 20px; padding: 32px; margin-bottom: 28px; box-shadow: 0 2px 20px rgba(0,0,0,0.04); transition: border-color 0.2s; }
        .tool-card-drag { border-color: rgba(74,124,126,0.5) !important; background: rgba(74,124,126,0.02); }
